...
public class MainUserInteraction extends Application {
    ...
    public void updateArrays(){
        ...
        sql = "SELECT Score FROM Users WHERE UserID = " + userID;
        for (String[] results :sqlDatabase.getFromDatabase(sql)){
            try{
                score = Integer.parseInt((sqlDatabase.getFromDatabase(sql).get(0)[0]));
            }
            catch(Exception e){
                score = 0;
            }
    ...
    public void start(Stage primaryStage) {
        ...
        xAxis3.setUpperBound(7);
        xAxis3.setTickUnit(1);
        NumberAxis yAxis3 = new NumberAxis();
        yAxis3.setLabel("Average number of calories consumed");
        BarChart barChart1 = new BarChart(xAxis3, yAxis3);
        XYChart.Series dataSeries4 = new XYChart.Series();
        dataSeries4.getData().add(new XYChart.Data("Monday", 0));
        dataSeries4.getData().add(new XYChart.Data("Tuesday", 0));
        dataSeries4.getData().add(new XYChart.Data("Wednesday", 0));
        dataSeries4.getData().add(new XYChart.Data("Thursday", 0));
        dataSeries4.getData().add(new XYChart.Data("Friday", 0));
        dataSeries4.getData().add(new XYChart.Data("Saturday", 0));
        dataSeries4.getData().add(new XYChart.Data("Sunday", 0));
        barChart1.getData().add(dataSeries4);
        ...
        gridPane5.add(barChart1, 1, 2 , 1, 1);
        ...
        button4.setOnAction(actionEvent -> {
            ...
            sql = "INSERT INTO User (UserID, Score) VALUES ('" + userID + "','" + score +"')";
            ...
            if(!datesEaten.isEmpty()){
                ...
                float[] avgCaloriesForEachDay = new float[7];
                int noDays = 0;
                for(int i = 0; i<caloriesPerDay.size(); i++){
                    avgCaloriesForEachDay[i%7] = avgCaloriesForEachDay[i%7] + caloriesPerDay.get(i);
                    noDays = noDays + 1;
                }
                for(int j = 0; j < 7; j++){
                    avgCaloriesForEachDay[j] = avgCaloriesForEachDay[j] / (noDays/7);
                }
                String[] daysNames = {"Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday",""};
                for(int i = 0; i< 7; i++){
                    dataSeries4.getData().add(new XYChart.Data(daysNames[i],avgCaloriesForEachDay[i]));
                }

                int weeklyCalorie = current;
                score = targetCaloriesForWeek - weeklyCalorie;
                primaryStage.setTitle("View targets and graphs");
                currentCalories.setText("You have consumed " + weeklyCalorie + " calories since the start of the week. Your current score is: " + score);
